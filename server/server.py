...
def url(urlStr):
    ...
    if "youtube.com" in urlStr:
        ...
        if rawTranscript!="":
            try:
                return categorize.function_a(rawTranscript,2,0)
            except BaseException:
                return '"101"'
        else:
            return '"001"'
        ...
        if theURL:
            try:
                return categorize.function_a(theURL,1,0)
            except BaseException:
                return '"102"'
        else: 
            print("Not working??????")
...
@app.route('/urlt/<path:urlStr>', methods=['GET'])
def urlt(urlStr):
    print(   urllib.parse.unquote(urlStr)   )
    if "youtube.com" in urlStr:
        rawTranscript = youtubeTranscript(urlStr)
        if rawTranscript!="":
            try:
                return categorize.function_a(rawTranscript,2,1)
            except BaseException:
                return '"101"'
        else:
            return '"001"'
        ...
        if theURL:
            try:
                return categorize.function_a(theURL,1,1)
            except BaseException:
                return '"102"'
        else: 
            print("Not working??????")
...
def runAlg(inputData):  
    if inputData:
        try:
            return categorize.function_a(inputData,0,0)
        except BaseException:
            return '"103"'
...
@app.route('/algt/<inputData>', methods=['GET'])
def runAlgt(inputData):  
    if inputData:
        try:
            return categorize.function_a(inputData,0,1)
        except BaseException:
            return '"103"'
